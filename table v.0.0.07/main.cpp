...
void highScores()
{
    for(int index=0;index<10;index++)
        for( int indexy=index+1;indexy<10;indexy++)
             if(jucatori[index].partide<jucatori[indexy].partide)
             swap(jucatori[index],jucatori[indexy]);
...
{
    ofstream fout;
    fout.open("clasament.in");
    for(int index=0;index <10;index++)
        fout<<jucatori[index].numeJucator<<" "<<jucatori[index].partide<<'\n';
    fout.close();
}
void newScore(char *nume_,int partida)
{
    for(int index=0;index<10;index++)
        if(partida>jucatori[index].partide)
    {
        for(int indexy=9;indexy>=index+1;indexy--)
        {
            jucatori[indexy].partide=jucatori[indexy-1].partide;
            jucatori[indexy].numeJucator=jucatori[indexy-1].numeJucator;
        }
        jucatori[index].partide=partida;
        jucatori[index].numeJucator=nume_;
        break;
    }
}
